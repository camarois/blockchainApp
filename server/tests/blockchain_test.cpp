...
TEST(BlockChainTest, loadFromEmptyDir) {
  ...
  Miner::BlockChainUPtr blockchain = Miner::BlockChain::fromDirectory(blockDir);
...
TEST(BlockChainTest, loadFromDir) {
  ...
  ASSERT_EQ(blockchain->lastBlock()->id(), 420U);
  ASSERT_EQ(blockchain->lastBlock()->nonce(), 0U);
  ASSERT_EQ(blockchain->lastBlock()->previousHash(), previous);
  ASSERT_EQ(blockchain->lastBlock()->data()[0], "Kathy Brousseau");
  ASSERT_EQ(blockchain->lastBlock()->data()[1], "Aya Bordeleau");
  ASSERT_EQ(blockchain->lastBlock()->data()[2], "Jeffrey Brisebois");
}
